...
public:
    ...
    objectName& at(unsigned int index);
    const objectName& at(unsigned int index) const;
...
namespace
{
    ...
    template <typename objectName>    
    void arrayCopy(objectName* target, objectName* source, unsigned int size)
    {
...
ArrayList<objectName>::ArrayList()
    : items{new objectName[initialCapacity]}, sz{0}, cap{initialCapacity}
{
...
template <typename objectName>
ArrayList<objectName>::ArrayList(const ArrayList& a)
    : items{new objectName[a.cap]}, sz{a.sz}, cap{a.cap}
{
...
template <typename objectName>
ArrayList<objectName>& ArrayList<objectName>::operator=(const ArrayList& a)
{

    if (this != &a)
    {
        objectName* newItems = new objectName[a.cap];
...
template <typename objectName>
objectName& ArrayList<objectName>::at(unsigned int index) 
{
    if (index < sz)
        return items[index];
    else
       throw  OutOfBoundsException("Index " + std::to_string(index) + " is out of bounds.");
    
    
}

template <typename objectName>
const objectName& ArrayList<objectName>::at(unsigned int index) const
{
    if (index < sz)
        return items[index];
    else
...
template <typename objectName>
void ArrayList<objectName>::add(const objectName& s)
{
    if (sz == cap)
    {
        int newCap = cap * 2 + 1;

        objectName* newItems = new objectName[newCap];
...
template <typename objectName>
unsigned int ArrayList<objectName>::size() const
{
    return sz;
}

template <typename objectName>
unsigned int ArrayList<objectName>::capacity() const
{
    return cap;
}
