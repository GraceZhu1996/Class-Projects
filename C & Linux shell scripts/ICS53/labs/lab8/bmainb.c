...
void search(char * pattern, int file_index, char * file_name, pthread_t *thread)
{
    ...
    if ((dir = opendir(file_name)) == NULL){
        ...
        //DO: fork_thread add_all_matching_lines(pattern, file_index, file_name);
            ...
            //switch(child){
             //   case 0:
                    ...
                    r = pthread_create((pthread_t*) thread, NULL, threadCall, args);               
                    if (r != 0){
                       free(args);
        ...
        while((ent = readdir(dir)) != NULL){
            ...
            search(pattern, file_index, fullpath(file_name, ent->d_name,  longpath), (pthread_t*) thread);
...
int main( int argc, char *argv[] ) // a parallel version of grep -r
{
    ...
    for (i=2; i < argc; i++){
        printf("start %d\n", i);
        search(argv[1], file_index, argv[i], &threads[file_index++]);
        printf("rend %d\n", i);
    //    sleep(1);
    }
